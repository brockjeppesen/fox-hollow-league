...
export const getProfile = query({
  ...
  handler: async (ctx, args) => {
    ...
    const playerRequests = allRequests.filter(
      (r) => r.playerId === args.id && r.playing
    );
    ...
    const topPartners = [...partnerCounts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const partnerHistory = await Promise.all(
      topPartners.map(async ([pid, count]) => {
        const p = await ctx.db.get(pid as any) as { name?: string } | null;
