            ...
            if (week?.slotCapacity) {
                ...
                if (capacity !== undefined) {
                    const allRequests = await ctx.db
                        ...
                        .withIndex("by_week", (q) => q.eq("weekId", args.weekId))
                        .collect();
                    const slotCount = allRequests.filter((r) => r.playing && r.timeSlot === args.timeSlot && !r.waitlisted && r.playerId !== args.playerId).length;
...
export const getWaitlist = query({
    ...
    handler: async (ctx, args) => {
        ...
        const enriched = await Promise.all(waitlisted.map(async (req) => {
            const player = await ctx.db.get(req.playerId);
            return {
                ...req,
                playerName: player?.name ?? "Unknown",
    ...
    handler: async (ctx, args) => {
        ...
        for (const week of openWeeks) {
            const request = await ctx.db
                .query("weeklyRequests")
                .withIndex("by_week_player", (q) => q.eq("weekId", week._id).eq("playerId", args.playerId))
                .first();
            // Get partner names if submitted
            let partnerNames = [];
            if (request?.wantsWith?.length) {
                const partners = await Promise.all(request.wantsWith.map((id) => ctx.db.get(id)));
...
export const bulkUpsert = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        for (const weekId of weekIds) {
            const existing = await ctx.db
                .query("weeklyRequests")
                .withIndex("by_week_player", (q) => q.eq("weekId", weekId).eq("playerId", data.playerId))
                .first();
            if (existing) {
...
export const getLastSubmission = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (latest.wantsWith?.length) {
            const partners = await Promise.all(latest.wantsWith.map((id) => ctx.db.get(id)));
            partnerNames = partners.filter(Boolean).map((p) => p.name);
        }
        return {
