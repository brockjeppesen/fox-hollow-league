...
export const generate = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Get all player data
        const players = await Promise.all(playingRequests.map((r) => ctx.db.get(r.playerId)));
        // Build player map
        ...
        for (const slotName of ["early", "mid", "late"]) {
            ...
            for (const player of remaining) {
                ...
                for (const partnerId of player.wantsWith) {
                    if (group.length >= 4)
                        break;
                    const partner = remaining.find((p) => p.playerId === partnerId && !used.has(p.playerId));
                    if (partner) {
                        group.push(partner);
        ...
        if (noPref.length > 0) {
            ...
            if (smallGroup) {
                for (const p of noPref)
                    smallGroup.players.push(p.playerId);
            }
            else {
                groups.push({
                    teeTime: allTimes[timeIdx % allTimes.length] || "TBD",
