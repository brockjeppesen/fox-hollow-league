...
export const listByWeek = query({
  ...
  handler: async (ctx, args) => {
    ...
    const weekTokens = allTokens.filter(
      (t) => t.weekId === args.weekId
    );

    const enriched = await Promise.all(
      weekTokens.map(async (t) => {
        const player = await ctx.db.get(t.playerId);
        const request = await ctx.db
          .query("weeklyRequests")
          .withIndex("by_week_player", (q) =>
            q.eq("weekId", args.weekId).eq("playerId", t.playerId)
          )
          .first();
  ...
  handler: async (ctx, args) => {
    ...
    for (const player of activePlayers) {
      const existing = await ctx.db
        .query("playerTokens")
        .withIndex("by_player_week", (q) =>
          q.eq("playerId", player._id).eq("weekId", args.weekId)
        )
        .first();
